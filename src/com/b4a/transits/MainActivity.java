...
public class MainActivity extends Activity implements B4AActivity{
	...
	/*	super.onCreate(savedInstanceState);
		...
		super.onCreate(savedInstanceState);
		if (isFirst) {
			processBA = new BA(this.getApplicationContext(), null, null, "com.b4a.transits", "MainActivity");
	 ...
	 @Override 
		public void onPause() {
	        ...
	        Msgbox.dismiss(true);
	        Common.Log("** Activity (MainActivity) Pause, UserClosed = " + activityBA.activity.isFinishing() + " **");
	...
	private void afterFirstLayout() {
		activityBA = new BA(this, layout, processBA, "com.b4a.transits", "MainActivity");
...
public TypefaceWrapper _v7 = null;
	...
	public static String  _btnchange_click() throws Exception{
		try {
			...
			if (Common.File.Exists(Common.File.getDirInternal(),"natal_data.txt") == true) { 
				
				_list1 = Common.File.ReadList(Common.File.getDirInternal(),"natal_data.txt");
				...
				_ret = BA.NumberToString(_dd.Show("Set your birth date","Tell me your birth date","OK","","",mostCurrent.activityBA,(Bitmap)(mostCurrent._v5.getObject())));

				_td.setHour(_h);
				_td.setMinute(_n);
				_td.setIs24Hours(true);
				_ret = BA.NumberToString(_td.Show("Set your birth time (use 24-hr time)","Tell me your birth time","OK","","",mostCurrent.activityBA,(Bitmap)(mostCurrent._v6.getObject())));
			}else {
				_dd.setYear(Common.DateTime.GetYear(Common.DateTime.getNow()));
				_dd.setMonth(Common.DateTime.GetMonth(Common.DateTime.getNow()));
				_dd.setDayOfMonth(Common.DateTime.GetDayOfMonth(Common.DateTime.getNow()));
				_ret = BA.NumberToString(_dd.Show("Set your birth date","Tell me your birth date","OK","","",mostCurrent.activityBA,(Bitmap)(mostCurrent._v5.getObject())));
			...
			Common.File.WriteList(Common.File.getDirInternal(),"natal_data.txt",_list1);
		}
		catch(Exception ex) {
	...
	public static String  _btndate_click() throws Exception{
		try {
			...
			if (Common.File.Exists(Common.File.getDirInternal(),"natal_data.txt") == false) { 
			...
			mostCurrent._edittext1.setInputType(mostCurrent._edittext1.INPUT_TYPE_NONE);
			...
			_natal_date = Common.NumberFormat(_d,2,0)+" "+_month_names[_m]+" "+BA.NumberToString(_y);
			_natal_time = BA.NumberToString(_h)+":"+anywheresoftware.b4a.keywords.Common.NumberFormat(_n,2,0);
			_natal_data_header = "   Natal date - "+_natal_date+" at "+_natal_time+" (tz="+_tz+")";
			_natal_data = "";
			{
				final double step148 = 1;
				final double limit148 = (int)(9);
				for (_i = 0; (step148 > 0 && _i <= limit148) || (step148 < 0 && _i >= limit148); _i += step148) {
					_vv4[_i] = _get_1_planet(_jd,_i);
					_natal_data = _natal_data+_p_names[_i]+" = "+_convert_longitude((float)(_vv4[_i]))+"   -   "+Common.NumberFormat2(_vv4[_i],1,4,4,false)+Common.CRLF;
				}
			};
			...
			_dd.setMonth(Common.DateTime.GetMonth(Common.DateTime.getNow()));
			_dd.setDayOfMonth(Common.DateTime.GetDayOfMonth(Common.DateTime.getNow()));
			_ret = BA.NumberToString(_dd.Show("Set the transit date","What transit date do you want?","OK","","",mostCurrent.activityBA,(Bitmap)(mostCurrent._v5.getObject())));

			_td.setHour(Common.DateTime.GetHour(Common.DateTime.getNow()));
			_td.setMinute(Common.DateTime.GetMinute(Common.DateTime.getNow()));
			_td.setIs24Hours(Common.True);
			_ret = BA.NumberToString(_td.Show("Set the transit time (use 24-hr time)","What transit time do you want to use?","OK","","",mostCurrent.activityBA,(android.graphics.Bitmap)(mostCurrent._v6.getObject())));
			_tz_offset = (float)(Double.parseDouble(_vv5()));
			_tod = (float)(_td.getHour()+_td.getMinute()/(double)60);
			_jd = _vv3.getJulDay(_dd.getYear(),_dd.getMonth(),_dd.getDayOfMonth(),_tod);
		...
		Log.v("ERROR AT BTN_DATE_CLICK","Error found at: "+ ex.getMessage());
	}
			return "";
		}
 ...
 * **/
		...
		try {
			...
			if (_flag_help == 1) { 
				if (Common.File.Exists(Common.File.getDirAssets(),"help.txt") == true) { 
					mostCurrent._edittext1.setText(Common.File.ReadString(Common.File.getDirAssets(),"help.txt"));
	...
	public static boolean  _check_for_overlap(float _angle,int _spacing) throws Exception{
		...
		_result = Common.False;
		{
			...
			for (_i = (int)(_angle-_spacing); (step703 > 0 && _i <= limit703) || (step703 < 0 && _i >= limit703); _i += step703) {
	...
	public static String  _convert_longitude(float _longitude) throws Exception{
		...
		if (true) 
			return Common.NumberFormat(_deg,(int)(2),(int)(0))+" "+_signs[_sign_num]+" "+Common.NumberFormat(_mn,(int)(2),(int)(0))+"'";
	...
	public static String  _vv7(double _jdx) throws Exception{
		...
		if (true) 
			return "Transit date - "+_dt_string+" at "+Common.NumberFormat(Common.Floor(_hh),2,0)+":"+Common.NumberFormat(_mm*60,2,0)+" (tz="+Common.NumberFormat2(_tz_offset,1,1,1,Common.False)+")";
		return "";
	}
	...
	public static float[]  _display_planet_glyph(float _our_angle,float _angle_to_use,int _radii) throws Exception{
		int _ch_pl_glyph = 0;
		int _cw_pl_glyph = 0;
		int _gap_pl_glyph = 0;
...
;
	...
	_xy[(int)(0)] = (float)(_center_pos_x+_offset_pos_x+((-_radii+_gap_pl_glyph)*Common.Cos(_angle_to_use)));
	_xy[(int)(1)] = (float)(_center_pos_y+_offset_pos_y+((_radii-_gap_pl_glyph)*Common.Sin(_angle_to_use)));
	if (true) 
		return _xy;
	return null;
	}
	...
	public static String  _display_wheel() throws Exception{
		...
		if (_aw>_ah || _ah==0 || _aw==0) { 
			mostCurrent._vv1.DrawText(mostCurrent.activityBA,"The wheel is not viewable in landscape mode.",(float)(50),(float)(104),Common.Typeface.DEFAULT,(float)(18),Common.Colors.White,BA.getEnumFromString(android.graphics.Paint.Align.class,"LEFT"));
			if (true) 
				return "";
		};

		if (_vv4[(int)(0)]==0 && _vv4[(int)(1)]==0 && _vvv1[(int)(0)]==0 && _vvv1[(int)(1)]==0 && _vv4[(int)(9)]==0 && _vvv1[(int)(9)]==0) { 
			if (true) return "";};
			_spacing = (int)(4);
			if (mostCurrent._edittext1.getVisible()==Common.False) { 
				...
				mostCurrent._vv1.DrawText(mostCurrent.activityBA,_transit_data_header,(float)(50),(float)(80),Common.Typeface.DEFAULT,(float)(18),Common.Colors.White,BA.getEnumFromString(Align.class,"LEFT"));
				mostCurrent._vv1.DrawText(mostCurrent.activityBA,_natal_data_header,(float)(50),(float)(104),Common.Typeface.DEFAULT,(float)(18),Common.Colors.White,BA.getEnumFromString(Align.class,"LEFT"));
				mostCurrent._vv1.DrawText(mostCurrent.activityBA,"Transit planets in",(float)(50),(float)(850),Common.Typeface.DEFAULT,(float)(18),Common.Colors.White,BA.getEnumFromString(Align.class,"LEFT"));
 				mostCurrent._vv1.DrawText(mostCurrent.activityBA,"Natal planets in",(float)(50),(float)(874),Common.Typeface.DEFAULT,(float)(18),Common.Colors.White,BA.getEnumFromString(Align.class,"LEFT"));
 				mostCurrent._vv1.DrawText(mostCurrent.activityBA,"magenta",(float)(200),(float)(850),Common.Typeface.DEFAULT,(float)(18),Common.Colors.Magenta,BA.getEnumFromString(Align.class,"LEFT"));
 				mostCurrent._vv1.DrawText(mostCurrent.activityBA,"cyan",(float)(190),(float)(874),Common.Typeface.DEFAULT,(float)(18),Common.Colors.Cyan,BA.getEnumFromString(Align.class,"LEFT"));
 		
 				{
 					final double step256 = 30,  limit256 = 330;
 					for (_i = (int)(0); (step256 > 0 && _i <= limit256) || (step256 < 0 && _i >= limit256); _i += step256) {
 						...
 						mostCurrent._vv1.DrawLine((float)(_x1+_center_pt_x),(float)(_y1+_center_pt_y),(float)(_x2+_center_pt_x),(float)(_y2+_center_pt_y),Common.Colors.Gray,(float)(Common.DipToCurrent(1)));
 					}
 				};

 				mostCurrent._vv1.DrawCircle((float)(_center_pt_x),(float)(_center_pt_y),(float)(_radius),Common.Colors.White,Common.False,(float)(Common.DipToCurrent((int)(2))));
 				mostCurrent._vv1.DrawCircle((float)(_center_pt_x),(float)(_center_pt_y),(float)(_inner_radius),Common.Colors.White,Common.False,(float)(Common.DipToCurrent((int)(2))));
 				_cw_sign_glyph = (int)(28);
 				_ch_sign_glyph = (int)(28);
 				_gap_sign_glyph = (int)(-32);
 				{
 					final double step268 = 1, limit268 = 12;
 					for (_i = 1; (step268 > 0 && _i <= limit268) || (step268 < 0 && _i >= limit268); _i += step268) {
 						_angle_to_use = (float)(_v0*(((_i-1)*30)+15));
 						_center_pos_x = (float)(-_cw_sign_glyph/(double)2);
 						_center_pos_y = (float)(_ch_sign_glyph/(double)2);
 						_offset_pos_x = (float)(_center_pos_x*Common.Cos(_angle_to_use));
 						_offset_pos_y = (float)(_center_pos_y*Common.Sin(_angle_to_use));
 						_x1 = (float)(_center_pos_x+_offset_pos_x+((-_radius+_gap_sign_glyph)*Common.Cos(_angle_to_use)));
 						_y1 = (float)(_center_pos_y+_offset_pos_y+((_radius-_gap_sign_glyph)*Common.Sin(_angle_to_use)));
 						mostCurrent._vv1.DrawText(mostCurrent.activityBA,String.valueOf(Common.Chr(_sign_glyph[_i])),(float)(_x1+_center_pt_x),(float)(_y1+_center_pt_y),(android.graphics.Typeface)(mostCurrent._v7.getObject()),(float)(28),Common.Colors.Green,BA.getEnumFromString(android.graphics.Paint.Align.class,"LEFT"));
 					}
 				};
 
 				{
 					final double step278 = 1, limit278 = 1;
 					for (_kk = (int)(0); (step278 > 0 && _kk <= limit278) || (step278 < 0 && _kk >= limit278); _kk += step278) {
 						{
 							final double step279 = 1;
 							final double limit279 = (int)(9);
 							for (_i = (int)(0); (step279 > 0 && _i <= limit279) || (step279 < 0 && _i >= limit279); _i += step279) {
 								if (_kk==0) { 
 									_sort[_i] = _vv4[_i];
 								}else {
 									_sort[_i] = _vvv1[_i];
...

 						{
 							...
 							for (_i = 0; (step287 > 0 && _i <= limit287) || (step287 < 0 && _i >= limit287); _i += step287) {
 								{
 									final double step288 = 1;
 									final double limit288 = 9;
 									for (_j = _i+1; (step288 > 0 && _j <= limit288) || (step288 < 0 && _j >= limit288); _j += step288) {
...

 						{
 							...
 							for (_i = 1; (step299 > 0 && _i <= limit299) || (step299 < 0 && _i >= limit299); _i += step299) {
 								_nopih[_i] = (int)(0);
 							}
 						};

 						{
 							...
 							for (_i = 0; (step302 > 0 && _i <= limit302) || (step302 < 0 && _i >= limit302); _i += step302) {
 								if (_kk==0) { 
 									_temp = Common.Floor(_vv4[_sort_pos[_i]]/(double)30)+1;
 								}else {
 									_temp = Common.Floor(_vvv1[_sort_pos[_i]]/(double)30)+1;
 								};
 						...
 						};
 						{
 							...
 							for (_i = 8; (step310 > 0 && _i <= limit310) || (step310 < 0 && _i >= limit310); _i += step310) {
 								if (_sort[_i]-_sort[(int)(_i+1)]>=20) { 
 									_pl_num = _sort_pos[_i];
 									if (_kk==0) { 
 										_house_of_pl = (int)(Common.Floor(_vv4[_pl_num]/(double)30)+1);
 									}else {
 										_house_of_pl = (int)(Common.Floor(_vvv1[_pl_num]/(double)30)+1);
 									};
 						...
 						if (_i>=0) { 
...

 							{
 								...
 								for (_i = _start_planet_idx; (step327 > 0 && _i <= limit327) || (step327 < 0 && _i >= limit327); _i += step327) {
...

 							{
 								...
 								for (_i = 9; (step332 > 0 && _i <= limit332) || (step332 < 0 && _i >= limit332); _i += step332) {
 									_sp[_cnt] = _sort_pos[_i];
 									_s[_cnt] = _sort[_i];
 									_cnt = (int)(_cnt-1);
 								}
...

 							{
 								...
 								for (_i = 0; (step337 > 0 && _i <= limit337) || (step337 < 0 && _i >= limit337); _i += step337) {
 									_sort_pos[_i] = _sp[_i];
 									_sort[_i] = _s[_i];
 								}
 							};
...

 						{
 							...
 							for (_i = 0; (step342 > 0 && _i <= limit342) || (step342 < 0 && _i >= limit342); _i += step342) {
 						...
 						{	
 							...
 							for (_i = 9; (step346 > 0 && _i <= limit346) || (step346 < 0 && _i >= limit346); _i += step346) {
 								_temp = _house_num;
 								if (_kk==0) { 
 									_house_num = (int)(Common.Floor(_vv4[_sort_pos[_i]]/(double)30)+1);
 									_offset_distance = 44;
 								}else {
 									_house_num = (int)(Common.Floor(_vvv1[_sort_pos[_i]]/(double)30)+1);
 									_offset_distance = 12;
 								};

 								if (_temp!=_house_num) { 
 									_planets_done = 1;};
 									_from_cusp = _vv6(_sort[_i]-((_house_num-1)*30));
 									_to_next_cusp = _vv6((_house_num*30)-_sort[_i]);
 									_next_cusp = _house_num*30;
 									_angle = _sort[_i];
 									_how_many_more_can_fit_in_this_house = (int)(Common.Floor(_to_next_cusp/(double)(_spacing+1)));
 									if (_nopih[_house_num]-_planets_done>=_how_many_more_can_fit_in_this_house) { 
 										_angle = _vv6(_next_cusp-((_nopih[_house_num]-_planets_done+1)*(_spacing+1)));
 									};
 									while (_check_for_overlap((float)(_angle),_spacing)==Common.True) {
 									...
 									_xy = _display_planet_glyph((float)(_our_angle),_angle_to_use,(int)(_radius-_offset_distance));
 									if (_kk==0) { 
 										mostCurrent._vv1.DrawText(mostCurrent.activityBA,String.valueOf(Common.Chr(_pl_glyph[_sort_pos[_i]])),(float)(_xy[(int)(0)]+_center_pt_x),(float)(_xy[(int)(1)]+_center_pt_y),(android.graphics.Typeface)(mostCurrent._v7.getObject()),(float)(32),Common.Colors.Cyan,BA.getEnumFromString(android.graphics.Paint.Align.class,"LEFT"));
 									}else {
 										mostCurrent._vv1.DrawText(mostCurrent.activityBA,String.valueOf(Common.Chr(_pl_glyph[_sort_pos[_i]])),(float)(_xy[(int)(0)]+_center_pt_x),(float)(_xy[(int)(1)]+_center_pt_y),(android.graphics.Typeface)(mostCurrent._v7.getObject()),(float)(32),Common.Colors.Magenta,BA.getEnumFromString(android.graphics.Paint.Align.class,"LEFT"));
 									};
 							}
 						};
 					}
...

 				{
 					...
 					for (_i = 0; (step381 > 0 && _i <= limit381) || (step381 < 0 && _i >= limit381); _i += step381) {
 						{
 							final double step382 = 1;
 							final double limit382 = 9;
 							for (_j = 0; (step382 > 0 && _j <= limit382) || (step382 < 0 && _j >= limit382); _j += step382) {
 								_q = _aspect_present[_i][_j];
 								if (_q>1) { 
 									_inner_radius = (int)(30);
 									_x1 = (float)((-_radius-10)*Common.Cos(_v0*_planet_angle[(int)(1)][_i]));
 									_y1 = (float)((_radius+10)*Common.Sin(_v0*_planet_angle[(int)(1)][_i]));
 									_x2 = (float)((-_radius+_inner_radius)*Common.Cos(_v0*_planet_angle[(int)(0)][_j]));
 									_y2 = (float)((_radius-_inner_radius)*Common.Sin(_v0*_planet_angle[(int)(0)][_j]));
 									if (_q==3 || _q==5) { 
 										mostCurrent._vv1.DrawLine((float)(_x1+_center_pt_x),(float)(_y1+_center_pt_y),(float)(_x2+_center_pt_x),(float)(_y2+_center_pt_y),Common.Colors.Green,(float)(Common.DipToCurrent((int)(2))));
 									}else if(_q==4 || _q==6) { 
 										mostCurrent._vv1.DrawLine((float)(_x1+_center_pt_x),(float)(_y1+_center_pt_y),(float)(_x2+_center_pt_x),(float)(_y2+_center_pt_y),Common.Colors.Red,(float)(Common.DipToCurrent((int)(2))));
 									}else if(_q==2) { 
 										mostCurrent._vv1.DrawLine((float)(_x1+_center_pt_x),(float)(_y1+_center_pt_y),(float)(_x2+_center_pt_x),(float)(_y2+_center_pt_y),Common.Colors.Yellow,(float)(Common.DipToCurrent((int)(4))));
 									};
 								};
 							}
 						};
	...
	public static String  _find_specific_report_paragraph(String _phrase_to_look_for,String _filename) throws Exception{
		String _txt = "";
		int _n = 0;
		int _x = 0;
		int _y = 0;
		...
		if (Common.File.Exists(Common.File.getDirAssets(),_filename)==Common.False) { 
			...
			if (_x==-1) { 
				...
				_y = _txt.indexOf("*",_x);
				_n = (int)(_y-_x-1);
				if (true) return _txt.substring((int)(_x-1),(int)(_x+_n-1));
				return "";
	}
	...
	public static double  _get_1_planet(double _jd,int _p_num) throws Exception{
		...
		_serr.Initialize();
		{
			...
			for (_i = 1; (step443 > 0 && _i <= limit443) || (step443 < 0 && _i >= limit443); _i += step443) {
				_serr.Append("0");
			}
		};
		_iflag = (int)(2+256);
		_ret_flag = _vvv2.swe_calc_ut(_jd,_p_num,_iflag,_xx,(java.lang.StringBuilder)(_serr.getObject()));
		if (true) return _xx[(int)(0)];
		return 0;
	}

	public static String  _get_transit_interps_for_this_day(double _jdx) throws Exception{
		...
		_transit_data = "";
		{
			...
			for (_i = 0; (step596 > 0 && _i <= limit596) || (step596 < 0 && _i >= limit596); _i += step596) {
				_vvv1[_i] = _get_1_planet(_jdx,_i);
				_transit_data = _transit_data+_p_names[_i]+" = "+_convert_longitude((float)(_vvv1[_i]))+"   -   "+Common.NumberFormat2(_vvv1[_i],(int)(1),(int)(4),(int)(4),Common.False)+Common.CRLF;
			}
		};
		
		if (mostCurrent._chkdebug.getChecked()==Common.True) { 
			mostCurrent._edittext1.setText((Object)(_transit_data_header+Common.CRLF+_transit_data+Common.CRLF+_natal_data_header+Common.CRLF+_natal_data+Common.CRLF));
		}else {
			mostCurrent._edittext1.setText((Object)(_transit_data_header+Common.CRLF+_natal_data_header+Common.CRLF));
		};
		...
		
		{
			...
			for (_i = 0; (step607 > 0 && _i <= limit607) || (step607 < 0 && _i >= limit607); _i += step607) {
				if (_i==1) { 
					if (true) continue;};
					{
						final double step609 = 1;
						final double limit609 = (int)(9);
						for (_j = 0; (step609 > 0 && _j <= limit609) || (step609 < 0 && _j >= limit609); _j += step609) {
							_aspect_present[_i][_j] = (int)(0);
							_da = BA.NumberToString(Common.Abs(_vv4[_j]-_vvv1[_i]));
							
							if ((double)(Double.parseDouble(_da))>180) { 
								_da = BA.NumberToString(360-(double)(Double.parseDouble(_da)));};
								_q = 1;
								if ((double)(Double.parseDouble(_da))<=_orb) { 
									_q = 2;
								}else if((double)(Double.parseDouble(_da))<=60+_orb && (double)(Double.parseDouble(_da))>=60-_orb) { 
									_q = 3;
								}else if((double)(Double.parseDouble(_da))<=90+_orb && (double)(Double.parseDouble(_da))>=90-_orb) { 
									_q = 4;
								}else if((double)(Double.parseDouble(_da))<=120+_orb && (double)(Double.parseDouble(_da))>=120-_orb) { 
									_q = 5;
								...
								if (_q>1) { 
									...
									_phrase_to_look_for = _p_names[_i]+_asp_names[_q]+_p_names[_j];
									_filename = _p_names[_i]+"_tr.txt";
									_txt = _find_specific_report_paragraph(_phrase_to_look_for,_filename);
									_any_text = "xxx";
									mostCurrent._edittext1.setText((Object)(mostCurrent._edittext1.getText()+_txt+Common.CRLF));
								};
						}
					};
			}
		...
		if ((_any_text).equals("")) { 
			mostCurrent._edittext1.setText((Object)(mostCurrent._edittext1.getText()+Common.CRLF+"You have no transits active at this time."+Common.CRLF));
		};
	...
	public static String  _vv0(double _jdx) throws Exception{
		...
		if (true)
			return Common.NumberFormat(_kw,2,0)+" "+_month_names[(int)(_jtw)]+" "+BA.NumberToString(_ikw);
		return "";
	}
...
_res = BA.NumberToString(Common.InputList(_list2,"Choose time zone",_idx,mostCurrent.activityBA));
_tz = (float)(0);
...
if (true) return BA.NumberToString(_tz);
return "";
}
	...
	public static String  _vv5() throws Exception{
		...
		_res = (int)(-Common.Round((_l-Common.DateTime.DateParse(_d))/(double)3600000));
		Common.DateTime.setDateFormat(_s);
		if (true) return BA.NumberToString(_res);
		return "";
	}
